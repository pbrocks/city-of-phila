...
class Help_Welcome_Menus {
	public function __construct() {
		...
		add_action( 'admin_menu', array( $this, 'wpe_code_sample_dashboard' ) );
	 ...
	 * @return array
	 */
	public function wpe_code_sample_dashboard() {
		global $code_sample_help_page;
		$slug                  = preg_replace( '/_+/', '-', __FUNCTION__ );
		$label                 = ucwords( preg_replace( '/_+/', ' ', __FUNCTION__ ) );
		$code_sample_help_page = add_dashboard_page( __( $label, 'wpe-code-sample' ), __( $label, 'wpe-code-sample' ), 'manage_options', $slug . '.php', array( $this, 'wpe_code_sample_response' ) );
	 ...
	 */
	public function wpe_code_sample_response() {
		global $code_sample_help_page;
		echo '<div class="wrap">';
		echo '<h2>' . ucwords( preg_replace( '/_+/', ' ', __FUNCTION__ ) ) . '</h2>';
		$response_tabs = new WPE_Code_Sample_Info();
		$hide_bouncing = get_user_meta( get_current_user_id(), 'hide_bouncing_arrow', true );
		if ( 'hide' !== $hide_bouncing ) {
			echo $hide_bouncing;
			$show_tabs = $response_tabs->wpe_code_sample_arrow();
		}
		$get_settings = $response_tabs->wpe_settings_page();
		$this->add_to_code_sample_dashboard();
		// $this->wpe_code_sample_footer();
		echo '</div>';
	}
	...
	public function initialize_code_sample_scripts() {
		wp_register_script( 'wpe-code-sample', plugins_url( 'js/wpe-code-sample.js', __DIR__ ), array( 'jquery' ), time(), true );
		wp_localize_script(
			'wpe-code-sample',
			'code_sample_object',
			array(
	...
	public function code_sample_welcome() {
		...
		wp_safe_redirect(
			add_query_arg(
				array(
					'page' => 'wpe-code-sample-dashboard.php',
				),
				admin_url( 'index.php' )
			)
		);
	...
	public function admin_add_help_tab() {
		...
		$screen->add_help_tab(
			array(
				...
				'title'   => __( 'Code Sample Help Tab One', 'wpe-code-sample' ),
				'content' => '<h3>' . __( 'Code Sample Help Tab', 'wpe-code-sample' ) . '</h3>' .
				'<p>' . __( 'Use this field to describe to the user what text you want on the help tab.', 'wpe-code-sample' ) . '</p>',
			)
		);
		$screen->add_help_tab(
			array(
				'id'      => 'code_sample_help_tab_2',
				'title'   => __( 'Code Sample Help Tab Two', 'wpe-code-sample' ),
				'content' => '<h3>' . __( 'Code Sample Help Tab', 'wpe-code-sample' ) . '</h3>' .
				'<p>' . __( 'Use this field to describe to the user what text you want on the help tab.', 'wpe-code-sample' ) . '</p>',
			)
		);
		$screen->add_help_tab(
			array(
				...
				'title'   => __( 'Request One', 'wpe-code-sample' ),
				'content' => '<h3>' . __( 'Scenario 1', 'wpe-code-sample' ) . '</h3>' .
				'<p>' . __( 'Marketing team members are having a hard time figuring out what page templates are being used on what pages. Write a plugin that lets authors easily see which template a page is using and also see only pages using a particular template.', 'wpe-code-sample' ) . '</p>',
			)
		);
		$screen->add_help_tab(
			array(
				'id'      => 'code_sample_help_tab_4',
				'title'   => __( 'Request Two', 'wpe-code-sample' ),
				'content' => '<h3>' . __( 'Code Sample Help Tab', 'wpe-code-sample' ) . '</h3>' .
